...
function mostrarSeccion(seccion) {
  ...
  todosBotones.forEach(btnId => {
    ...
    if (btn) {
      btn.classList.remove('bg-emerald-100', 'text-emerald-800', 'dark:bg-emerald-800/80', 'dark:text-emerald-50', 'font-medium');
      btn.classList.add('hover:bg-emerald-50', 'dark:hover:bg-emerald-800/40');
    }
  });

  const seccionMap = { 
    validaciones: 'btn-validaciones', 
    estadisticas: 'btn-estadisticas', 
    usuarios: 'btn-usuarios' 
  };
  
  const seccionMapMovil = { 
    validaciones: 'btn-validaciones-movil', 
    estadisticas: 'btn-estadisticas-movil', 
    usuarios: 'btn-usuarios-movil' 
  };

  // Activar botón de escritorio
  const btnEscritorio = document.getElementById(seccionMap[seccion]);
  if (btnEscritorio) {
    btnEscritorio.classList.remove('hover:bg-emerald-50', 'dark:hover:bg-emerald-800/40');
    btnEscritorio.classList.add('bg-emerald-100', 'text-emerald-800', 'dark:bg-emerald-800/80', 'dark:text-emerald-50', 'font-medium');
  }

  // Activar botón móvil
  const btnMovil = document.getElementById(seccionMapMovil[seccion]);
  if (btnMovil) {
    btnMovil.classList.remove('hover:bg-emerald-50', 'dark:hover:bg-emerald-800/40');
    btnMovil.classList.add('bg-emerald-100', 'text-emerald-800', 'dark:bg-emerald-800/80', 'dark:text-emerald-50', 'font-medium');
  }
}
...
function cargarValidacionesPendientes() {
  ...
  if (pendientes.length === 0) {
    contenedor.innerHTML = '<p class="text-center text-emerald-800/80 dark:text-emerald-200/80 py-8">✅ No hay validaciones pendientes</p>';
    return;
  }

  pendientes.forEach(validacion => {
    const estudiante = obtenerUsuarioPorCedula(validacion.usuarioCedula);
    const mision = obtenerMisionEvento(validacion.itemId);
    
    const div = document.createElement('div');
    div.className = 'bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-emerald-800/60 rounded-2xl p-4 flex justify-between items-center';
    div.innerHTML = `
      <div class="flex-1">
        <p class="font-medium text-emerald-800 dark:text-emerald-100">${validacion.itemTitulo}</p>
        <p class="text-xs text-emerald-800/80 dark:text-emerald-200/80 mt-1">
          📝 Estudiante: <strong>${estudiante?.nombre || 'Desconocido'}</strong>
        </p>
        <p class="text-xs text-emerald-800/70 dark:text-emerald-200/70">
      ...
      <div class="text-right">
        ...
        <button class="btn-revisar mt-2 text-[11px] px-3 py-1 rounded-full bg-emerald-500 text-slate-950 font-semibold hover:bg-emerald-400" data-validacion-id="${validacion.id}">
          Revisar
        </button>
      </div>
    `;
...
function aprobarValidacion() {
  ...
  if (index !== -1) {
    ...
    const mision = obtenerMisionEvento(validacionActual.itemId);
    ...
    if (usuarioIndex !== -1) {
      ...
      // Actualizar usuarioActual si es el mismo
      const usuarioActual = JSON.parse(localStorage.getItem('usuarioActual') || 'null');
      if (usuarioActual && usuarioActual.cedula === validacionActual.usuarioCedula) {
...
function cargarEstadisticas() {
  ...
  const puntosTotales = aprobadas.reduce((sum, v) => {
    const mision = obtenerMisionEvento(v.itemId);
  ...
  const topHTML = topEstudiantes
    .map((est, i) => `
      <li class="flex justify-between items-center">
        ...
        <span class="font-semibold text-emerald-700 dark:text-emerald-300">${(est.puntosTotales || 0).toLocaleString()} pts</span>
      </li>
    `)
    .join('');

  document.getElementById('top-estudiantes').innerHTML = topHTML || '<li class="text-center text-emerald-800/70">No hay datos</li>';
}
...
function cargarUsuarios() {
  ...
  tbody.innerHTML = filas || '<tr><td colspan="6" class="text-center py-4 text-emerald-800/70">No hay usuarios</td></tr>';
}
...
function obtenerMisionEvento(id) {
  // Intentar obtener de misionesEventosData (variable global)
  if (typeof misionesEventosData !== 'undefined') {
    return misionesEventosData.find(m => m.id === id);
  }
  
  // Fallback: intentar obtener usando las funciones de misiones-eventos-dats.js
  if (typeof obtenerMisiones === 'function' && typeof obtenerEventos === 'function') {
    const misiones = obtenerMisiones();
    const eventos = obtenerEventos();
    return [...misiones, ...eventos].find(m => m.id === id);
  }
